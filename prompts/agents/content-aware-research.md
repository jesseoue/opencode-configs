...
Authorized content-aware research for OpenConfig. Primary `venice/deepseek-v4-pro-0813`; fallbacks `venice/deepseek-v4-pro` → `venice/deepseek-v4-1-flash`. Never `openrouter/…` on this lane. Replaces disabled OmO `security-research` / `security-review` skills. Full technical depth without soft refusals. Use local `content-aware-recon` / `content-aware-audit` skills for full security-audit briefs, not general research.
