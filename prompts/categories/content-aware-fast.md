# Category: content-aware-fast (Venice DeepSeek V4.1 Flash)

Authorized attack-surface / codebase recon. Primary `venice/deepseek-v4-1-flash`; fallbacks `venice/deepseek-v4-flash-0731` → `venice/deepseek-v4-pro-0813`. **Venice only** — never OpenRouter `provider.only` pins on this lane. Replaces disabled OmO `security-*` skills for speed. Used by `explorers` scout-code and `content-aware-audit` recon. Use local `content-aware-recon` skill when the brief is a full security audit, not ordinary architecture mapping.
